...
static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	len = (size_t)snprintf(NULL, 0, "%s/.dlitefs.db", dev_name);
	db_path = kmalloc(len, GFP_KERNEL);
	snprintf(db_path, len, "%s/.dlitefs.db", dev_name);

	err = unqlite_open(&dbp, db_path, UNQLITE_OPEN_CREATE);
	kfree(db_path);
	if (err != UNQLITE_OK) {
		printk(KERN_ERR "wrapfs: unable to create mapping db\n");
		goto out_free;
	}

	WRAPFS_SB(sb)->dbp = dbp;
...
void wrapfs_shutdown_super(struct super_block *sb) {
	int err;

	err = unqlite_close(WRAPFS_SB(sb)->dbp);
	if (err != UNQLITE_OK)
		printk(KERN_ERR "failed to close berkeleydb\n");
