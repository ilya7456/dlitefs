...
static int wrapfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	/*
	 * trap uid changes, write to db instead and clear the ATTR_UID flag
	 */
	if (lower_ia.ia_valid & ATTR_UID) {
		len = snprintf(NULL, 0, "%lu.uid", inode->i_ino);
		db_key = kmalloc(len, GFP_KERNEL);
		snprintf(db_key, len, "%lu.uid", inode->i_ino);

		len = snprintf(NULL, 0, "%hu", (unsigned short)ia->ia_uid.val);
		db_value = kmalloc(len, GFP_KERNEL);
		snprintf(db_value, len, "%hu", (unsigned short)ia->ia_uid.val);

		err = unqlite_kv_store(WRAPFS_SB(dentry->d_sb)->dbp, db_key, -1, db_value, len);
		kfree(db_key);
		kfree(db_value);
		if (err != UNQLITE_OK)
			goto out;

		lower_ia.ia_valid &= ~ATTR_UID;
	}

	/*
	 * trap gid changes, write to db instead and clear the ATTR_GID flag
	 */
	if (lower_ia.ia_valid & ATTR_GID) {
		len = snprintf(NULL, 0, "%lu.gid", inode->i_ino);
		db_key = kmalloc(len, GFP_KERNEL);
		snprintf(db_key, len, "%lu.gid", inode->i_ino);

		len = snprintf(NULL, 0, "%hu", (unsigned short)ia->ia_gid.val);
		db_value = kmalloc(len, GFP_KERNEL);
		snprintf(db_value, len, "%hu", (unsigned short)ia->ia_gid.val);
			  ...
			  struct kstat *stat)
{
	...
	err = unqlite_kv_fetch(WRAPFS_SB(dentry->d_sb)->dbp, db_key, -1, NULL, &len);
	if (err != UNQLITE_OK) {
		kfree(db_key);
		goto no_uid;
	}
	...
	db_value = kmalloc(len, GFP_KERNEL);
	err = unqlite_kv_fetch(WRAPFS_SB(dentry->d_sb)->dbp, db_key, -1, db_value, &len);
	kfree(db_key);
	if (err != UNQLITE_OK) {
		kfree(db_value);
		goto no_uid;
	...
	uid = (unsigned short)simple_strtoul(db_value, NULL, 0);
	kfree(db_value);
	stat->uid = KUIDT_INIT(uid);
...
no_uid:
	...
	gid = (unsigned short)simple_strtoul(db_value, NULL, 0);
	kfree(db_value);
	stat->gid = KGIDT_INIT(gid);
...
static int
wrapfs_setxattr(struct dentry *dentry, struct inode *inode, const char *name, const void *value,
